...
private[contest] abstract class Btss(val id: String, val name: String) {

  def score(boards: Boards, playerBtsss: Btss.PlayerBtssScores): Btss.PlayerBtssScores

}

private[contest] object Btss {

  case class BtssScore(btss: Btss, score: Double)
  case class PlayerBtssScore(player: Player, btsss: List[BtssScore] = List.empty) {
    def +(btssScore: BtssScore) = copy(player, btsss :+ btssScore)
  }
  case class PlayerBtssScores(players: List[PlayerBtssScore]) {

    def opponentPlayer(player: Player, board: Board) = {
  ...
  case object Opponent extends Btss("opponent", "对手分") {
  ...
  case object Mid extends Btss("mid", "中间分") {
  ...
  case object Sauber extends Btss("sauber", "索伯分") {
  ...
  // 对手没走棋不算分
  case object Vict extends Btss("vict", "胜局数") {
        ...
        playerBtssScores.players.map { playerBtssScore =>
          ...
          val score =
            boards.filter(_.contains(player.no)).foldLeft(0D) {
              case (s, b) => {
                val opponent = playerBtssScores.opponentPlayer(player, b)
                val opponentNoStart = opponent.roundOutcome(b.roundNo).??(_ == Board.Outcome.NoStart)
  ...
  case object Res extends Btss("res", "直胜") {
  ...
  case object No extends Btss("no", "序号") {
  ...
  def apply(id: String): Btss = byId get id err s"Bad Btss $id"
...
case class Btsss(list: List[Btss]) {
