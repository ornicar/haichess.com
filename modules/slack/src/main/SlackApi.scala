...
final class SlackApi(
  ...
  def publishEvent(event: Event): Funit = event match {
    ...
    case TournamentName(userName, tourId, tourName) => client(SlackMessage(
      ...
      text = s"${userLink(userName)} created ${link(s"https://haichess.com/tournament/$tourId", s"$tourName Arena")}",
  ...
  def publishRestart =
    if (isProd) publishInfo("Haichess 已经重启")
  ...
  private def userLink(name: String): String = link(s"https://haichess.com/@/$name?mod", name)
  private def userLink(user: User): String = userLink(user.username)
  private def userNotesLink(name: String) = link(s"https://haichess.com/@/$name?notes", "notes")
  private def broadcastLink(id: String, name: String) = link(s"https://haichess.com/broadcast/-/$id", name)
  private def gameLink(path: String) = link(s"https://haichess.com/$path", path)
  private val chatPanicLink = link("https://haichess.com/mod/chat-panic", "Chat Panic")

  private val userRegex = lila.common.String.atUsernameRegex.pattern
  private val userReplace = link("https://haichess.com/@/$1?mod", "$1")
  ...
  def deployPre: Funit =
    if (isProd) client(SlackMessage(
      username = "deployment",
      icon = "rocket",
      text = "Haichess 将会在一分钟之内更新",
      channel = rooms.general
    ))
    else client(SlackMessage(
      username = stage.name,
      icon = stage.icon,
      text = "Stage 将会在一分钟之内更新",
      channel = rooms.general
    ))
