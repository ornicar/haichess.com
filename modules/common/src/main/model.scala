...
case class CellphoneAddress(value: String) extends AnyVal with StringValue {
  override def toString = s"${value.take(3)}****${value.takeRight(4)}"
}

object CellphoneAddress {

  val regex = """^(13[0-9]|14[5-9]|15[0-3,5-9]|16[2,5,6,7]|17[0-8]|18[0-9]|19[1,3,5,8,9])\d{8}$""".r

  def matches(str: String): Boolean = regex find str

  def from(str: String): Option[CellphoneAddress] =
    matches(str) option CellphoneAddress(str)
}
