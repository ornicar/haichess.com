...
object Form {
  ...
  def futureDateTime = inTheFuture(jodaDate("yyyy-MM-dd HH:mm"))
  ...
  object ISODateTime2 {
    ...
    val isoDateTime = jodaDate(dateTimePattern, DateTimeZone.UTC)
    implicit val dateTimeFormat = jodaDateTimeFormat(dateTimePattern)
    val isoDate = of[org.joda.time.DateTime](formatter)
  }
  object ISOTime {
    val timePattern = "HH:mm"
    val formatter = jodaDateTimeFormat(timePattern, DateTimeZone.UTC)
    val isoDateTime = jodaDate(timePattern, DateTimeZone.UTC)
    implicit val timeFormat = jodaDateTimeFormat(timePattern)
    val isoTime = of[org.joda.time.DateTime](formatter)
  }
  object ISODate {
