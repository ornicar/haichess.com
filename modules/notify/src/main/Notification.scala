...
case class CoachApply(
    userId: lila.user.User.ID
) extends NotificationContent("coachApply")

case class CoachApproved(
    userId: lila.user.User.ID,
    status: String
) extends NotificationContent("coachApproved")

case class TeamApply(
    userId: lila.user.User.ID,
    teamId: String
) extends NotificationContent("teamApply")

case class TeamApproved(
    teamId: String,
    status: String
) extends NotificationContent("teamApproved")

case class InvitedToTeam(
    invitedBy: InvitedToTeam.InvitedBy,
    clazzName: InvitedToTeam.TeamName,
    clazzId: InvitedToTeam.TeamId
) extends NotificationContent("inviteTeam")

object InvitedToTeam {
  case class InvitedBy(value: String) extends AnyVal with StringValue
...
case class InvitedToClazz(
    invitedBy: InvitedToClazz.InvitedBy,
    clazzName: InvitedToClazz.ClazzName,
    clazzId: InvitedToClazz.ClazzId
) extends NotificationContent("invitedClazz")

object InvitedToClazz {
  case class InvitedBy(value: String) extends AnyVal with StringValue
  case class ClazzName(value: String) extends AnyVal with StringValue
  case class ClazzId(value: String) extends AnyVal with StringValue
}

/*
object ContestJoined {
  case class Id(value: String) extends AnyVal with StringValue
  case class Name(value: String) extends AnyVal with StringValue
  case class LateMinute(value: String) extends AnyVal with StringValue
}*/
