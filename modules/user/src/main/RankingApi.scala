...
final class RankingApi(
  ...
  private[user] def topPuzzlePerf(nb: Int): Fu[List[User.LightPerf]] =
    UserRepo.topPerfUser(PerfType.Puzzle, nb).map { list =>
      list.map { user =>
        User.LightPerf(
          user = user.light,
          perfKey = PerfType.Puzzle.key,
          rating = user.perfs.puzzle.intRating,
          progress = user.perfs.puzzle.progress
        )
      }
    }
