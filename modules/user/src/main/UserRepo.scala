...
import lila.common.{ ApiVersion, CellphoneAddress, EmailAddress, NormalizedEmailAddress }
...
object UserRepo {
  ...
  def find(doc: BSONDocument) = coll.find(doc)
  ...
  def optionsByOrderedIds(ids: Seq[ID]): Fu[List[Option[User]]] =
    coll.optionsByOrderedIds[User, User.ID](ids, readPreference = ReadPreference.secondaryPreferred)(_.id)
  ...
  def topPerfUser(pt: PerfType, n: Int): Fu[List[User]] =
    coll.find(enabledSelect).sort($doc(s"${F.perfs}.${pt.key}.gl.r" -> -1)).list[User](n)

  def setProfile(id: ID, profile: Profile): Funit =
    coll.update(
      $id(id),
      $set(
        s"${F.profile}.head" -> profile.head,
        s"${F.profile}.country" -> profile.country,
        s"${F.profile}.bio" -> profile.bio,
        s"${F.profile}.firstName" -> profile.firstName,
        s"${F.profile}.lastName" -> profile.lastName,
        s"${F.profile}.fideRating" -> profile.fideRating,
        s"${F.profile}.uscfRating" -> profile.uscfRating,
        s"${F.profile}.ecfRating" -> profile.ecfRating,
        s"${F.profile}.links" -> profile.links,
        s"${F.profile}.province" -> profile.province,
        s"${F.profile}.city" -> profile.city,
        s"${F.profile}.realName" -> profile.realName,
        s"${F.profile}.sex" -> profile.sex,
        s"${F.profile}.birthyear" -> profile.birthyear,
        /*s"${F.profile}.mobile" -> profile.mobile,*/
        s"${F.profile}.wechat" -> profile.wechat
      )
    ).void

  def setCurrentLevel(id: ID, lv: Level): Funit =
    coll.update(
      $id(id) ++ $doc(s"${F.levels}.level" -> lv.level),
      $set(
        s"${F.levels}.$$.current" -> 1,
        s"${F.levels}.$$.name" -> lv.name,
        s"${F.levels}.$$.time" -> lv.time,
        s"${F.levels}.$$.result" -> lv.result
      )
    ) flatMap { result =>
        if (result.n == 0) {
          coll.update(
            $id(id) ++ $doc(s"${F.levels}.current" -> 1),
            $set(
              s"${F.levels}.$$.current" -> 0
            )
          ) >> coll.update(
              $id(id),
              $addToSet(F.levels -> Levels.levelBSONHandler.write(lv))
            ).void
        } else funit
      }
  ...
  def create(
    ...
    !nameExists(username) flatMap {
      _ ?? {
        val doc = newUser(username, passwordHash, email, blind, mobileApiVersion, mustConfirmEmail, level) ++
          ("len" -> BSONInteger(username.size))
        coll.insert(doc) >> setCurrentLevel(normalize(username), Level(level = level, current = 1)) >> {
          named(normalize(username)) flatMap { userOption =>
            userOption.?? { u =>
              Env.current.memberApi.signup(u)
            } inject userOption
          }
        }
      }
  ...
  def userIdsSameLevel(level: String): Fu[List[User.ID]] =
    coll.find(
      $doc(s"${F.levels}" $elemMatch $doc("level" -> level, "current" -> 1)) ++ enabledSelect,
      $doc(F.id -> true)
    )
      .list[Bdoc](10000, ReadPreference.secondaryPreferred)
      .map {
        _ flatMap { _.getAs[User.ID](F.id) }
      }
  ...
  def setRole(id: ID, role: String) = coll.update($id(id), $addToSet("roles" -> role)).void

  def removeRole(id: ID, role: String) = coll.update($id(id), $pull("roles" -> role)).void
  ...
  def cellphone(id: ID): Fu[Option[CellphoneAddress]] = coll.find(
    $id(id),
    $doc(
      F.cellphone -> true
    )
  ).uno[Bdoc].map { _ ?? { doc => doc.getAs[String](F.cellphone) map { CellphoneAddress(_) } } }

  def setCellphone(id: ID, cellphone: String): Funit = {
    coll.update(
      $id(id),
      $set(F.cellphone -> cellphone)
    ).void
  }

  def cellphoneExits(id: ID, cellphone: String) =
    coll.exists($doc(F.cellphone -> cellphone, F.id $ne id))

  def wechatExits(user: User, mobile: String) =
    coll.exists($doc(F.wechat -> mobile, F.id $ne user.id))

  def setTeam(id: ID, teamId: String): Funit = {
    coll.update(
      $id(id),
      $set(F.teamId -> teamId)
    ).void
  ...
  def setTeamEmpty(id: ID, teamId: String): Funit = {
    coll.update(
      $id(id),
      $unset(F.teamId)
    ).void
  ...
  private def newUser(
    ...
    implicit def perfsHandler = Perfs.perfsInitBSONHandler
    import lila.db.BSON.BSONJodaDateTimeHandler
    ...
    $doc(
      ...
      F.perfs -> Perfs.init(FormSelect.Level.levelRating(level)),
      ...
      F.profile -> Profile.profileBSONHandler.write(Profile.default.withLevelKey(level))*/
    ) ++ {
  ...
  def goldExpiredAndSilverNotExpired(): Fu[List[User.ID]] = {
    coll.find(
      $doc(
        F.mcode -> MemberLevel.Gold.code,
        s"${F.mlevels}.${MemberLevel.Gold.code}.expireAt" $lt DateTime.now,
        s"${F.mlevels}.${MemberLevel.Silver.code}.expireAt" $gt DateTime.now
      ),
      $doc(
        F.id -> true
      )
    ).list[Bdoc](100, ReadPreference.secondaryPreferred).map {
        _.flatMap {
          _.getAs[User.ID]("_id")
        }
      }
  ...
  def goldExpiredAndSilverExpired(): Fu[List[User.ID]] = {
    coll.find(
      $doc(
        F.mcode -> MemberLevel.Gold.code,
        s"${F.mlevels}.${MemberLevel.Gold.code}.expireAt" $lt DateTime.now,
        $or(
          s"${F.mlevels}.${MemberLevel.Silver.code}" $exists false,
          s"${F.mlevels}.${MemberLevel.Silver.code}.expireAt" $lt DateTime.now
        )
      ),
      $doc(
        F.id -> true
  ...
  def silverExpired(): Fu[List[User.ID]] = {
    coll.find(
      $doc(
        F.mcode -> MemberLevel.Silver.code,
        s"${F.mlevels}.${MemberLevel.Silver.code}.expireAt" $lt DateTime.now
      ),
      $doc(
        F.id -> true
  ...
  def filterCoachOrTeam(ids: Set[String]): Fu[List[User.ID]] = {
    coll.find(
      $doc(
        F.id $in ids,
        F.roles $in List("ROLE_COACH", "ROLE_TEAM")
      ),
      $doc(
        F.username -> true
      )
