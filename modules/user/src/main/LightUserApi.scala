  ...
  implicit val lightUserBSONReader = new BSONDocumentReader[LightUser] {
    ...
    def read(doc: BSONDocument) = {
      LightUser(
        ...
        member = doc.getAs[Bdoc](F.member).flatMap(_.getAs[String]("code")),
        head = doc.getAs[Bdoc](F.profile).flatMap(_.getAs[String]("head")),
        roles = ~doc.getAs[List[String]](F.roles)
      )
    }
  }

  val projection = $doc(F.username -> true, F.title -> true, s"${F.plan}.active" -> true, F.mcode -> true, s"${F.profile}.head" -> true, s"${F.roles}" -> true)
}
