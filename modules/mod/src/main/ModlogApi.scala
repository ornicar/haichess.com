...
final class ModlogApi(coll: Coll) {
  ...
  def enableTeam(mod: String, name: String, desc: String) = add {
    Modlog(mod, none, Modlog.enableTeam, details = s"$name / $desc".take(200).some)
  }

  def disableTeam(mod: String, name: String, desc: String) = add {
  ...
  def teamCertificationProcess(mod: String, user: String, teamName: String, approve: Boolean) = add {
    val action = if (approve) Modlog.teamCertificationApprove else Modlog.teamCertificationRejected
    Modlog(mod, user.some, action, details = Some(teamName take 140))
  }
