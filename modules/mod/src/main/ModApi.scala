...
final class ModApi(
  ...
  def addPermissions(mod: Mod, username: String, permissions: Set[Permission]): Funit = withUser(username) { user =>
    val finalPermissions = Permission(user.roles) ++ permissions.filter(Granter.canGrant(mod.user, _))
  ...
  def removePermissions(mod: Mod, username: String, permissions: Set[Permission]): Funit = withUser(username) { user =>
  ...
  def setPermissions(mod: Mod, username: String, permissions: Set[Permission]): Funit = withUser(username) { user =>
    ...
    UserRepo.setRoles(user.id, finalPermissions.map(_.name).toList) >> {
      lilaBus.publish(lila.hub.actorApi.mod.SetPermissions(user.id, user.roles, finalPermissions.map(_.name).toList), 'setPermissions)
      logApi.setPermissions(mod, user.id, permissions.toList)
    }
  }
