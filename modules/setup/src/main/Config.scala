...
trait BaseConfig {
  var variantList = List(chess.variant.Standard.id, FromPosition.id)
  val variants = variantList
  val variantDefault = chess.variant.Standard
  val variantsWithFen = variantList
  val aiVariants = variantList
  val variantsWithVariants = variantList
  val variantsWithFenAndVariants = variantList
