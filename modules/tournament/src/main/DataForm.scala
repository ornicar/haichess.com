...
final class DataForm {
  ...
  private lazy val create = Form(mapping(
    ...
    .verifying("无效的对局时间", _.validClock)
    .verifying("15s variant games cannot be rated", _.validRatedUltraBulletVariant)
    .verifying("增加锦标赛持续时间, 或减少对局时间", _.sufficientDuration)
    .verifying("减少锦标赛持续时间, 或增加对局时间", _.excessiveDuration))
}
...
object DataForm {
  ...
  val minuteChoices = options(minutes, "%d 分钟")

  val waitMinutes = Seq(1, 2, 3, 5, 10, 15, 20, 30, 45, 60)
  val waitMinuteChoices = options(waitMinutes, "%d 分钟")
