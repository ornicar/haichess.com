      ...
      cached ranking tour flatMap { ranking =>
        tour.system.pairingSystem.createPairings(tour, users, ranking).flatMap {
          ...
          case pairings if nowMillis - startAt > 1200 =>
            pairingLogger.warn(s"Give up making https://haichess.com/tournament/${tour.id} ${pairings.size} pairings in ${nowMillis - startAt}ms")
          ...
          if (time > 100)
            pairingLogger.debug(s"Done making https://haichess.com/tournament/${tour.id} in ${time}ms")
        }
      }
    }
  }
