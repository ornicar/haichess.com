...
object Condition {
  ...
  object DataForm {
    ...
    val maxRatingChoices = ("", "无限制") ::
      options(maxRatings, "%d 分").toList.map { case (k, v) => k.toString -> v }
    ...
    val minRatingChoices = ("", "无限制") ::
      options(minRatings, "%d 分").toList.map { case (k, v) => k.toString -> v }
