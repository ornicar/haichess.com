...
final class PgnDump(
  ...
  def filename(study: Study): String = {
    ...
    fileR.replaceAllIn(
      s"haichess_study_${slugify(study.name.value)}_by_${ownerName(study)}_${date}.pgn", ""
    )
  }
  ...
  def filename(study: Study, chapter: Chapter): String = {
    ...
    fileR.replaceAllIn(
      s"haichess_study_${slugify(study.name.value)}_${slugify(chapter.name.value)}_by_${ownerName(study)}_${date}.pgn", ""
    )
  }
  ...
  private def annotatorTag(study: Study) =
    Tag(_.Annotator, s"https://haichess.com/@/${ownerName(study)}")
...
private[study] object PgnDump {
  ...
  case class MoveWithNode(turn: chessPgn.Move, node: Node)
  case class TurnWithNode(number: Int, white: Option[MoveWithNode], black: Option[MoveWithNode])

  def toTurnsWithNode(root: Node.Root): List[TurnWithNode] = toTurnsWithNode(root.mainline, root.children.variations)

  def toTurnsWithNode(line: List[Node], variations: Variations): List[TurnWithNode] = (line match {
    case Nil => Nil
    case first :: rest if first.ply % 2 == 0 => TurnWithNode(
      number = 1 + (first.ply - 1) / 2,
      white = none,
      black = MoveWithNode(node2move(first, variations), first).some
    ) :: toTurnsWithNodeFromWhite(rest, first.children.variations)
    case l => toTurnsWithNodeFromWhite(l, variations)
  }) filterNot (t => t.white.isEmpty && t.black.isEmpty)

  def toTurnsWithNodeFromWhite(line: List[Node], variations: Variations): List[TurnWithNode] =
    (line grouped 2).foldLeft(variations -> List.empty[TurnWithNode]) {
      case ((variations, turns), pair) => pair.headOption.fold(variations -> turns) { first =>
        pair.lift(1).getOrElse(first).children.variations -> (toTurnWithNode(first, pair lift 1, variations) :: turns)
      }
    }._2.reverse

  def toTurnWithNode(first: Node, second: Option[Node], variations: Variations) = TurnWithNode(
    number = first.fullMoveNumber,
    white = MoveWithNode(node2move(first, variations), first).some,
    black = second map { se => MoveWithNode(node2move(se, first.children.variations), se) }
  )
