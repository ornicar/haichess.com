...
case class Study(
  ...
  def isStudent = visibility == Study.Visibility.Student
  def isTeam = visibility == Study.Visibility.Team
...
object Study {
  ...
  object Visibility {
    ...
    val byKey = List(Private, Student, Team, Unlisted, Public).map { v => v.key -> v }.toMap
  }
  ...
  def make(user: User, from: From, id: Option[Study.Id] = None, name: Option[Name] = None, settings: Option[Settings] = None) = {
    ...
    Study(
      _id = id | makeId,
      name = name | Name(s"${user.username} 的研习"),
      members = StudyMembers(Map(user.id -> owner)),
      position = Position.Ref(Chapter.Id(""), Path.root),
      ownerId = user.id,
      visibility = if (user.hasStudyPrivate) Visibility.Private else Visibility.Public,
