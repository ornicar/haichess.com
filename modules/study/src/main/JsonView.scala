...
final class JsonView(
  ...
  def chapterInfo(study: Study, c: Chapter) = Json.obj(
    "name" -> s"${study.name}：${c.name}",
    "color" -> c.setup.orientation.name,
    "root" -> c.root.fen.value,
    "moves" -> JsArray(
      PgnDump.toTurnsWithNode(c.root) map { twn =>
        Json.obj(
          "index" -> twn.number,
