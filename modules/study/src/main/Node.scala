...
case class Node(
  ...
  object depthFix {
    def toMap(map: scala.collection.mutable.Map[String, Node], parentPath: Option[String]): scala.collection.mutable.Map[String, Node] =
      children.nodes.zipWithIndex.foldLeft(map) {
        case (m, (n, i)) => {
          val path = parentPath.fold(s"$i") { p => s"$p:$i" }
          n.depthFix.toMap(map, path.some)
          m.put(path, n)
          m
        }
      }

    def toChildren(k: String, map: Map[String, Node]): Node.Children = Node.Children(
      map.filterKeys(("^" + k + ":[0-9]*$").r.pattern.matcher(_).find()).toList.sortWith { (n1, n2) => n1._1 < n2._1 } map {
        case (k, n) => n.copy(children = n.depthFix.toChildren(k, map))
      } toVector
    )
  }
...
object Node {
  ...
  object Root {
    ...
    object depthFix {
      ...
      def toMap(children: Children): Map[String, Node] =
      ...
      def toChildren(map: Map[String, Node]): Node.Children = Node.Children(
        map.filterKeys(("^[0-9]*$").r.pattern.matcher(_).find()).toList.sortWith { (n1, n2) => n1._1 < n2._1 } map {
          case (k, n) => n.copy(children = n.depthFix.toChildren(k, map))
        } toVector
      )
    }
