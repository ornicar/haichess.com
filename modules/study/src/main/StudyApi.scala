...
final class StudyApi(
  ...
  def importGame(data: StudyMaker.ImportGame, user: User): Fu[Option[Study.WithChapter]] = (data.form.as match {
    case DataForm.importGame.AsNewStudy => {
      studyMaker(data, user) flatMap { res =>
        if (!res.study.isPrivate || (res.study.isPrivate && user.hasStudyPrivate)) {
        ...
        } else fufail("Forbidden, user can not create private study")
      }
    }
    ...
    Settings.UserSelection.allows(prev.settings.cloneable, prev, me.id.some) ?? {
      chapterRepo.orderedByStudy(prev.id).flatMap { chapters =>
        ...
        newChapters.headOption.map(study1.rewindTo) ?? { study =>
          if (!study.isPrivate || (study.isPrivate && me.hasStudyPrivate)) {
            studyRepo.insert(study) >>
              newChapters.map(chapterRepo.insert).sequenceFu >>- {
                chat ! lila.chat.actorApi.SystemTalk(
                  ...
                  s"Cloned from haichess.com/study/${prev.id}"
                )
              } inject study.some
          } else fufail("Forbidden, user can not clone private study")
        }
      }
    }
  ...
  def editStudy(byUserId: User.ID, studyId: Study.Id, data: Study.Data) = sequenceStudy(studyId) { study =>
    data.settings.ifTrue(study isOwner byUserId) ?? { settings =>
      UserRepo.byId(byUserId) flatMap {
        case None => funit
        case Some(u) => {
          ...
          if (!newStudy.isPrivate || (newStudy.isPrivate && u.hasStudyPrivate)) {
  ...
  private val likesTop5Cache = asyncCache.single[List[Study]](
    name = "study.likesTop5",
    f = studyRepo.publicLikes,
    expireAfter = _.ExpireAfterWrite(30 minute)
  )

  def likesTop5: Fu[List[Study]] = likesTop5Cache.get
  ...
  def chapterPgn(id: String, chapterId: String): Fu[Option[String]] =
    byIdWithChapter(Study.Id(id), Chapter.Id(chapterId)).map { swc =>
      swc.map { x =>
        pgnDump.ofChapter(x.study, x.chapter).toString
      }
    }
