...
object BSONHandlers {
  ...
  private[study] implicit def NodeRootBSONHandler: BSON[Root] = new BSON[Root] {
    def reads(r: Reader) = Root(
      ...
      clock = r.getO[Centis]("l"),
      crazyData = r.getO[Crazyhouse.Data]("z"),
      children = r.get[Node.Children]("n")(ChildrenBSONHandler)
    )
  ...
  implicit def ChildrenBSONHandler = new BSONHandler[Bdoc, Node.Children] {
    private val mapHandler = BSON.MapDocument.MapHandler[String, Node]
    def read(b: Bdoc) = try {
      Root.depthFix.toChildren(mapHandler.read(b))
    } catch {
    ...
    def write(x: Node.Children) = try {
      mapHandler write Root.depthFix.toMap(x)
    } catch {
      case e: StackOverflowError =>
        println(s"study handler ${e.toString}")
        $doc()
