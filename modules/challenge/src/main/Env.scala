...
final class Env(
  ...
  system.lilaBus.subscribeFun(
    ...
    'apptProcessBus
      ...
      case lila.hub.actorApi.game.ApptCompleteBus(gameId, user, time, source) => if (source == "challenge") api.apptComplete(gameId, user, time)
      case lila.hub.actorApi.game.ApptProcessBus(gameId, user, source) => if (source == "challenge") api.apptProcess(gameId, user)
    }
