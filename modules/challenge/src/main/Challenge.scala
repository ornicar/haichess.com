...
case class Challenge(
    ...
    mode: Mode,
    appt: Boolean,
    apptStartsAt: Option[DateTime],
    apptMessage: Option[String],
  ...
  def contains(user: User.ID) = userIds.exists(_.contains(user))
  ...
  def canCancelOrDecline = status == Status.Created || status == Status.Processed
...
object Challenge {
  ...
  object Status {
    ...
    case object Processed extends Status(50)
    val all = List(Created, Offline, Canceled, Declined, Accepted, Processed)
