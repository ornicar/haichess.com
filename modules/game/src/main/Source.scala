...
object Source {
  ...
  case object Contest extends Source(id = 13)

  val all = List(Lobby, Friend, Ai, Api, Tournament, Contest, Position, Import, Simul, Relay, Pool)
  val byId = all map { v => (v.id, v) } toMap

  val searchable = List(Lobby, Friend, Ai, Position, Import, Tournament, Contest, Simul, Pool)
