...
import chess.{ Castles, Centis, CheckCount, Clock, Color, Mode, MoveMetrics, MoveOrDrop, Situation, Speed, Status, Timestamp, Game => ChessGame }
...
case class Game(
  ...
  def contestId = metadata.contestId
  def contestCanLateMinute = metadata.contestCanLateMinute
  def simulId = metadata.simulId

  def isTournament = tournamentId.isDefined
  def isContest = contestId.isDefined
  def isSimul = simulId.isDefined
  def isMandatory = isTournament || isContest || isSimul
  def isClassical = perfType contains Classical
  def nonMandatory = !isMandatory
  def isAppt = metadata.appt
  def isApptNotComplete = metadata.appt && !metadata.apptComplete
  def isApptFriend = isAppt && (fromFriend || fromPosition)

  def hasChat = !isTournament && !isContest && !isSimul && nonAi
  ...
  def created = status == Status.Created
  ...
  def playerCanProposeTakeback(color: Color) =
    started && playable && !isTournament && !isContest && !isSimul &&
  ...
  def contestRoundStart(movedAt: DateTime) = clock.map { clock =>
    val newClock = if (clock.isRunning) clock else clock.copy(timer = Some(Timestamp(movedAt.getMillis)))
    Progress(this, copy(
      status = Status.Started,
      chess = chess.copy(clock = newClock.some),
      movedAt = movedAt
    ), List(
      Event.Start,
  ...
  def apptStart(movedAt: DateTime) = clock.map { clock =>
    //val newClock = if (clock.isRunning) clock else clock.copy(timer = Some(Timestamp(movedAt.getMillis)))
    Progress(this, copy(
      status = Status.Started,
      /*chess = chess.copy(clock = newClock.some),*/
  ...
  def accountable = playedTurns >= 2 || isTournament || isContest
  ...
  def ratingVariant =
    if ((isTournament || isContest) && variant.fromPosition) Standard
    else variant
  ...
  private def outoftimeClock(withGrace: Boolean): Boolean = clock ?? { c =>
    started && playable && (bothPlayersHaveMoved || isSimul || isApptFriend) && {
  ...
  def expirable =
    !bothPlayersHaveMoved && (source.exists(Source.expirable.contains) || isApptFriend) && playable && nonAi && hasClock

  def contestExpirable =
    status.is(Status.Started) && !bothPlayersHaveMoved && isContest && playable && nonAi && hasClock

  def timeBeforeExpiration: Option[Centis] = expirable || contestExpirable option {
    Centis.ofMillis(movedAt.getMillis - nowMillis + timeForFirstMove.millis + ((metadata.contestCanLateMinute | 0) * 60 * 1000 - 2000).toLong).nonNeg
  }
  ...
  def isBeingPlayed = !isPgnImport && !finishedOrAborted && !status.is(Status.Created)
  ...
  def withContestId(id: String) = copy(metadata = metadata.copy(contestId = id.some))

  def withContestCanLateMinutes(contestCanLateMinute: Int) = copy(metadata = metadata.copy(contestCanLateMinute = contestCanLateMinute.some))

  def withSimulId(id: String) = copy(metadata = metadata.copy(simulId = id.some))

  def withAppt(appt: Boolean) = copy(metadata = metadata.copy(appt = appt))

  def withApptWhenCreate(appt: Boolean) =
    if (appt) {
      copy(metadata = metadata.copy(appt = true))
    } else this
  ...
  def secondsToStart = (movedAt.getSeconds - nowSeconds).toInt atLeast 0
...
object Game {
  ...
  def make(
    ...
    NewGame(Game(
      ...
      createdAt = createdAt,
      movedAt = movedAt | createdAt
    ))
  }

  def makeSyntheticGame =
    lila.game.Game.make(
      chess = chess.Game(
  ...
  object BSONFields {
    ...
    val contestId = "cid"
    val contestCanLateMinute = "clm"
    val simulId = "sid"
    val appt = "appt"
    val apptComplete = "apptc"
    val apptReminded = "apptr"
    val apptLaunched = "apptl"
    val tvAt = "tv"
    val winnerColor = "w"
    val winnerId = "wid"
    val initialFen = "if"
