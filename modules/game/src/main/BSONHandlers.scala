...
object BSONHandlers {
  ...
  implicit val gameBSONHandler: BSON[Game] = new BSON[Game] {
    ...
    def reads(r: BSON.Reader): Game = {
      ...
      Game(
        ...
        metadata = Metadata(
          ...
          contestId = r strO F.contestId,
          contestCanLateMinute = r intO F.contestCanLateMinute,
          simulId = r strO F.simulId,
          analysed = r boolD F.analysed,
          appt = r boolD F.appt,
          apptComplete = r boolD F.apptComplete
        )
      )
    }
    ...
    def writes(w: BSON.Writer, o: Game) = BSONDocument(
      ...
      F.contestId -> o.metadata.contestId,
      F.contestCanLateMinute -> o.metadata.contestCanLateMinute,
      F.simulId -> o.metadata.simulId,
      F.analysed -> w.boolO(o.metadata.analysed),
      F.appt -> w.boolO(o.metadata.appt),
      F.apptComplete -> w.boolO(o.metadata.apptComplete)
    ) ++ {
