...
object GameRepo {
  ...
  def removeByIds(ids: List[ID], userId: User.ID) =
    coll.remove(
      $inIds(ids) ++ Query.imported(userId)
    ).void
  ...
  def incBookmarksByIds(ids: List[ID], value: Int) =
    coll.update($inIds(ids), $inc(F.bookmarks -> value), multi = true).void
  ...
  def random: Fu[Option[Game]] =
    coll.find($doc(Query.turnsGt(20)))
      .sort(Query.sortCreated)
  ...
  def importedTags(userId: String): Fu[Set[String]] =
    coll.distinct[String, Set](s"${F.pgnImport}.tags", Query.imported(userId).some)

  def start(id: Game.ID): Funit =
    coll.update(
      $doc(F.id -> $id(id)),
      $set(F.status -> chess.Status.Started.id)
    ).void

  def setMoveAt(id: ID, movedAt: DateTime): Funit =
    coll.update(
      $id(id),
      $set(F.movedAt -> movedAt)
    ).void

  def apptComplete(id: Game.ID, movedAt: DateTime): Funit =
    coll.update(
      $id(id),
      $set(F.apptComplete -> true, F.movedAt -> movedAt)
    ).void

  def gameStartBatch(ids: List[Game.ID], movedAt: DateTime): Funit =
    coll.update(
      $doc(F.id -> $in(ids: _*)),
      $set(F.status -> chess.Status.Started.id, F.movedAt -> movedAt),
      multi = true
    ).void

  def gameLaunchBatch(ids: List[Game.ID]): Funit =
    coll.update(
      $doc(F.id -> $in(ids: _*)),
      $set(F.apptLaunched -> true),
      multi = true
    ).void

  def apptPending: Fu[List[Game]] =
    coll.find(
      $doc(
        F.status -> chess.Status.Created.id,
        F.appt -> true,
        F.apptComplete -> true,
        F.apptLaunched $ne true,
        F.source -> $in(Source.Friend.id, Source.Position.id),
        F.movedAt -> ($lte(DateTime.now) ++ $gte(DateTime.now minusMinutes 3))
      )
    ).list[Game]()

  def apptRemind: Fu[List[Game]] =
    coll.find(
      $doc(
        F.status -> chess.Status.Created.id,
        F.appt -> true,
        F.apptComplete -> true,
        F.apptReminded $exists false,
        F.source -> $in(Source.Friend.id, Source.Position.id),
        F.movedAt -> ($lte(DateTime.now plusMinutes 1) ++ $gte(DateTime.now))
      )
    ).list[Game]()

  def setApptRemind(id: Game.ID): Funit =
    coll.update(
      $id(id),
      $set(
        F.apptReminded -> true
