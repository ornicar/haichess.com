...
import lila.game.{ Game, GameRepo, PlayerRef, Pov }
import lila.hub.actorApi.map.Tell
import lila.hub.actorApi.round.{ Abort, FishnetPlay, Resign }
...
final class Env(
  ...
  private[round] def scheduleExpiration(game: Game): Unit = game.started ?? {
    game.timeBeforeExpiration foreach { centis =>
      system.scheduler.scheduleOnce((centis.millis + 1000).millis) {
        if (game.isApptNotComplete) {
          roundMap.tell(game.id, actorApi.round.AbortForce)
        } else {
  ...
  lazy val apptRoundApi = new ApptRoundApi(roundMap, bus)
