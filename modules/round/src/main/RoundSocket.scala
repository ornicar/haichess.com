...
private[round] final class RoundSocket(
  ...
  private var contestId = none[String] // until set, to listen to standings
  private var contestCanLateMinute = none[Int]
  ...
  def receiveSpecific: Trouper.Receive = ({
    ...
    case SetGame(Some(game)) =>
      ...
      contestCanLateMinute = game.contestCanLateMinute
    ...
    case StartGame(game) =>
      this ! SetGame(game.some)
      if (game.isContest || game.isApptFriend) {
        notifyRoundStartReload(game.id)
      }
  ...
  def notifyContestGone(color: Color, gone: Boolean): Unit = {
    notifyOwner(!color, "contestGone", gone)
  }

  def notifyRoundStartReload(gameId: Game.ID): Unit = {
    members.foreachValue { m =>
      m push makeMessage("roundStartReload", gameId)
    }
  }
