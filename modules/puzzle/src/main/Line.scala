...
object Line {
  ...
  private def toPiotr(uci: String) = chess.Pos.doubleKeyToPiotr(uci take 4) match {
    case Some(m) => s"$m${uci drop 4}"
    case _ => sys error s"Invalid uci move notation: $uci"
  }

  def toJson(lines: Lines): JsObject = JsObject(lines map {
    case Win(move) => toPiotr(move) -> JsBoolean(true)
    case Retry(move) => toPiotr(move) -> JsBoolean(false)
    case Node(move, more) => toPiotr(move) -> toJson(more)
  })

  def toJson2(lines: Lines): JsObject = JsObject(lines map {
    case Win(move) => move -> JsString("成功")
    case Retry(move) => move -> JsString("重试")
    case Node(move, more) => move -> toJson2(more)
  })
}
