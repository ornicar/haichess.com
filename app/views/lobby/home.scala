...
object home {
  ...
  def apply(
    ...
    deferJs = true
      ...
      /*,
       div(cls := "lobby__side")( ctx.blind option h2("Highlights"),
         ...
         ctx.me map { u =>
           ...
           trans.xIsAFreeYLibreOpenSourceChessServer("Haichess", a(cls := "blue", href := routes.Plan.features)(trans.really.txt())),
           " ",
           a(href := "/about")(trans.aboutX("Lichess"), "...")
         ) ),
       featured map { g =>
         div(cls := "lobby__tv")(gameFen(Pov first g, tv = true),
       ...
       ctx.noBot option bits.underboards(tours, simuls, leaderboard, tournamentWinners, puzzle),
