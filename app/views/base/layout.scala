...
import lila.common.{ ContentSecurityPolicy, Lang }
...
object layout {
  ...
  object bits {
    ...
    /*val topComment = raw("""<!-- Haichess is open source! See https://github.com/ornicar/lila -->""")*/
  ...
  private def userIcon(me: lila.user.User) = {
    if (me.isTeam) teamIcon
    else if (me.isCoach) coachIcon
    else if (me.memberOrDefault.isGoldAvailable) goldIcon
    else if (me.memberOrDefault.isSilverAvailable) silverIcon
    else ""
  }
  ...
  private val zenToggle = raw("""<a data-icon="E" id="zentog" class="text fbt active">禅意模式</a>""")
  private def dasher(me: lila.user.User) = raw(s"""<div class="dasher"><a id="user_tag" class="toggle link" data-icon="r">${me.username} ${userIcon(me)}</a><div id="dasher_app" class="dropdown"></div></div>""")
...
<a href="${routes.Auth.login}?referrer=${ctx.req.path}" class="signin button button-empty">${trans.signIn().render}</a>""")
  ...
  private val clinputLink = a(cls := "link")(span(dataIcon := "查"))
  ...
  private val dataHeads = attr("data-heads")
    ...
    htmlTag(ctx.lang)(
      ...
      head(
        ...
        if (isProd && !isStage) frag(
          st.headTitle(fullTitle | s"$title • haichess.com")
        )
        else st.headTitle(s"[dev] ${fullTitle | s"$title • haichess.com"}"),
        cssTag("site"),
        ...
        link(id := "favicon", rel := "shortcut icon", href := staticUrl("images/favicon2-32-white.png"), `type` := "image/x-icon"),
        ...
        style := zoomable option s"--zoom:${ctx.zoom}"
          ...
          ctx.me.map { me =>
            div(
              ...
              dataHeads := safeJsonValue(JsArray(ctx.onlineFriends.heads.map {
                case (u, h) => Json.obj(
                  "u" -> u,
                  "h" -> h
                )
    ...
    footer(
      div(cls := "foot")(
        div(cls := "about")(
          ...
          a(href := "/faq")("常见问题"),
          a(href := "/contact")("联系我们"),
          a(href := "/terms-of-service")("用户协议"),
          a(href := "/privacy")("隐私政策")
        ),
        div(cls := "copyright")(
          span("© 嗨棋科技", nbsp, nbsp, a(href := "https://beian.miit.gov.cn")("京ICP备19042715号-1")),
          a(href := "http://www.beian.gov.cn/portal/registerSystemInfo?recordcode=11010102003898")(
            img(src := staticUrl("ba.png")), nbsp, "京公网安备 11010102003898号"
          )
        )
      )
    )
...
<label for="tn-tg" class="hbg"><span class="hbg__in"></span></label>""")
    ...
    private def reports(implicit ctx: Context) = isGranted(_.SeeReport) option
      frag(split, a(cls := "link data-count link-center", title := "Moderation", href := routes.Report.list, dataCount := reportNbOpen, dataIcon := ""))

    private def teamRequests(implicit ctx: Context) = ctx.teamNbRequests > 0 option
      frag(split, a(cls := "link data-count link-center", href := routes.Team.requests, dataCount := ctx.teamNbRequests, dataIcon := "f", title := trans.teams.txt()))
      ...
      header(id := "top")(
        div(cls := "site-title-nav")(
          ...
          h1(cls := "site-title")(
            ...
            a(href := routes.Home.home)(i("标"), "haichess.com", span(""))
          ),
    ...
    private val split = span(cls := "split")
  }
}
