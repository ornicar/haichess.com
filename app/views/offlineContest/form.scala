...
object form {
  ...
  def update(id: OffContest.ID, form: Form[_], teams: List[lila.team.Team], clazzs: ClazzIdsWithNames)(implicit ctx: Context) =
    layout(
      form,
      teams,
      clazzs,
  ...
  def layout(form: Form[_], teams: List[lila.team.Team], clazzs: ClazzIdsWithNames, title: String, url: Call)(implicit ctx: Context) = views.html.base.layout(
    title = title,
    ...
    moreJs = frag(
      ...
      div(cls := "contest__form box box-pad", dataPublic := JsArray(teams.filter(_.certified).map(t => Json.obj("id" -> t.id, "name" -> t.name))).toString, dataTeamInner := JsArray(teams.map(t => Json.obj("id" -> t.id, "name" -> t.name))).toString, dataClazzInner := JsArray(clazzs.map(t => Json.obj("id" -> t._1, "name" -> t._2))).toString)(
        h1(title),
        postForm(cls := "form3", action := url)(
          ...
          form3.split(
            ...
            form3.group(form("organizer"), raw("主办方"), half = true)(form3.select(_, teams.map(t => t.id -> t.name)))
          ),
          form3.split(
