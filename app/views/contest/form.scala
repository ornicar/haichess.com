...
object form {
  ...
  def update(id: Contest.ID, form: Form[_], teams: List[lila.team.Team], clazzs: List[(Clazz, Boolean)])(implicit ctx: Context) =
    layout(
      form,
      teams,
      clazzs,
  ...
  def clone(form: Form[_], teams: List[lila.team.Team], clazzs: List[(Clazz, Boolean)])(implicit ctx: Context) =
    layout(
      form,
      teams,
      clazzs,
  ...
  private def layout(
    ...
    clazzs: List[(Clazz, Boolean)],
    title: String,
    url: play.api.mvc.Call,
    id: Option[Contest.ID] = None
            ...
            div(cls := "panels")(
              div(cls := List("panel basics" -> true, "active" -> (form.errors.isEmpty || form.errors.headOption ?? (_.key.isEmpty) || errorTabActive(form, "basics"))))(
                ...
                form3.split(
                  ...
                  form3.group(form("basics.organizer"), raw("主办方"), half = true)(f => frag(
                ...
                form3.split(
                  div(cls := "form-group form-half form-flex")(
                    form3.checkbox(form("basics.rated"), raw("有积分"), help = raw("对局将会计分<br>并影响棋手的积分").some),
                    form3.checkbox(form("basics.teamRated"), raw("记录俱乐部等级分"), help = raw("影响棋手所在俱乐部的积分").some, klass = "none")
                  ),
