...
object timeline {
  ...
  private def entry(e: lila.timeline.Entry)(implicit ctx: Context) = frag(
    e.decode.map[Frag] {
      ...
      case TeamJoin(userId, teamId) => trans.xJoinedTeamY(userIdLink(userId.some, withOnline = false), teamLinkById(teamId, withIcon = false))
      case TeamCreate(userId, teamId) => trans.xCreatedTeamY(userIdLink(userId.some, withOnline = false), teamLinkById(teamId, withIcon = false))
