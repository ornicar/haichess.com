...
object bits {
  ...
  def authLinks(me: User, active: String, order: lila.study.Order)(implicit ctx: Context) = {
    ...
    frag(
      ...
      a(activeCls("mineMember"), href := routes.Study.mineMember(order.key))("我参与的研习"),
      a(activeCls("minePublic"), href := routes.Study.minePublic(order.key))("我公共的研习"),
      a(activeCls("minePrivate"), href := routes.Study.minePrivate(order.key))("我私有的研习"),
      a(activeCls("mineLikes"), href := routes.Study.mineLikes(order.key))("我收藏的研习")
    )
  }
