...
object bits {
  ...
  def create(me: User)(implicit ctx: Context) = views.html.site.message(
    title = "Become a haichess streamer",
    icon = Some(""),
    back = false,
  ...
  def pic(s: lila.streamer.Streamer, u: User, size: Int = 300)(implicit ctx: Context) = s.picturePath match {
    case Some(path) => img(
      ...
      alt := s"${u.titleUsername} haichess streamer"
    )
    case _ => img(
      width := size,
      height := size,
      cls := "default picture",
      src := staticUrl("images/coach-default.svg"),
...
""")
  ...
  def rules = ul(cls := "streamer-rules")(
    li("Be listed as a haichess streamer."),
    li(title := "For example: Blitz battle on haichess.com")("Get bumped up the top of the list when you stream with the keyword \"haichess.com\" in the stream title."),
    li("Notify your haichess followers when you start streaming."),
