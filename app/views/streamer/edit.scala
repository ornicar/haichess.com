                ...
                (ctx.is(s.user) && s.streamer.listed.value) option div(
                  ...
                  dataIcon := (if (granted) "E" else "")
                    ...
                    if (granted) frag(
                      ...
                      a(href := routes.Streamer.index())("haichess streamers list"), "."
                    )
                    else frag(
                ...
                postForm(cls := "form3", action := s"${routes.Streamer.edit}${!ctx.is(s.user) ?? s"?u=${s.user.id}"}")(
                  isGranted(_.Streamers) option div(cls := "mod")(
                    ...
                    form3.split(
                      ...
                      if (granted)
                        form3.checkbox(form("approval.featured"), raw("Feature on haichess homepage"), help = modsOnly, half = true)
                      else
                  ...
                  form3.split(
                    form3.group(form("name"), raw("Your streamer name on haichess"), help = raw("Keep it short: 20 characters max").some, half = true)(form3.input(_)),
