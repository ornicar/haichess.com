...
object bits {
  ...
  def details(c: Challenge)(implicit ctx: Context) = frag(
    ...
    c.appt option appt(c)
  )

  def appt(c: Challenge)(implicit ctx: Context) = div(cls := "appt")(
    h2("自由约棋"),
    c.apptStartsAt.map { startsAt =>
      p(startsAt.toString("yyyy年MM月dd日 HH:mm"))
    },
    c.apptMessage.map { message =>
      p("留言：", message)
    }
  )
