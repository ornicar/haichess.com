      ...
      main(cls := "page-menu")(
        ...
        div(cls := "rating-stats page-menu__content box box-pad")(
          h1(trans.weeklyPerfTypeRatingDistribution(views.html.base.bits.mselect(
            "variant-stats",
            span(perfType.name),
            PerfType.leaderboardableShort map { pt =>
              a(
            ...
            ctx.me.flatMap(_.perfs(perfType).glicko.establishedIntRating).map { rating =>
              lila.user.Stat.percentile(data, rating) match {
                case (under, sum) => div(
                  ...
                  trans.youAreBetterThanPercentOfPerfTypePlayers(raw(s"""<strong>${"%.1f" format (if (sum == 0) { 0.0 } else { under * 100.0 / sum })}%</strong>"""), perfType.name)
                )
              }
            } getOrElse div(
