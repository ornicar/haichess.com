      ...
      openGraph = lila.app.ui.OpenGraph(
        ...
        main(cls := "box page-small search")(
          ...
          div(cls := "search__result", id := "results")(
            paginator.map { pager =>
              val permalink = a(cls := "permalink", href := routes.Search.index(), rel := "nofollow")("永久链接")
              if (pager.nbResults > 0) frag(
                div(cls := "search__status box__pad")(
                  strong(pager.nbResults.localize, " 局对局"), " • ",
                  permalink
                ),
              ...
              else div(cls := "search__status box__pad")(strong("没有找到对局"), " • ", permalink)
            }
          )
        )
      }
