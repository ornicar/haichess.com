...
object widgets {
  ...
  def apply(
    ...
    st.article(cls := "game-row paginated", dataId := g.id, dataHref := gameLink(g, firstPlayer.color, ownerLink))(
      ...
      div(cls := "game-row__infos")(
        div(cls := "header", dataIcon := bits.gameIcon(g))(
          div(cls := "header__text")(
            ...
            g.contestId map { contestId =>
              frag(separator, contestLink(contestId))
            },
        ...
        if (g.turns > 0) {
          val len = /*if (showFullMoves) (1 + (80 - 1) / 2) else*/ (1 + (16 - 1) / 2)
        ...
        g.pgnImport.flatMap(_.user).map { user =>
          div(cls := "metadata")("PGN import by ", userIdLink(user.some, withOnline = false))
        }
      ) /*,
      showCheckbox option input(tpe := "checkbox", cls := "cbx-game", id := s"cbx-${g.id}", name := "cbx-game", value := g.id)*/
    )
  }
  ...
  def showClock(clock: chess.Clock) = frag(clock.config.show)
