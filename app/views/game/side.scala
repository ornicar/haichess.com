...
object side {
  ...
  def apply(
    ...
    contest: Option[lila.contest.Board.FullInfo],
    simul: Option[lila.simul.Simul],
    userTv: Option[lila.user.User] = None,
    bookmarked: Boolean
  )(implicit ctx: Context): Option[Frag] = ctx.noBlind option frag(
    meta(pov, initialFen, tour, contest, simul, userTv, bookmarked),
  ...
  def meta(
    ...
    div(cls := "game__meta")(
        ...
        div(cls := "game__meta__players")(
          game.players.map { p =>
            div(cls := s"player color-icon is ${p.color.name} text")(
              playerLink(p, withOnline = true, withDiff = true, withBerserk = true)
            )
          }
        )
      ),
      ...
      (game.isApptFriend && game.created) option
        st.section(cls := "game__appt")(
          div(cls := "clock", dataTime := game.secondsToStart)(div(cls := "time")(game.startStatus))
        ),
      ...
      contest.map { c =>
        st.section(cls := "game__contest")(
          div(
            a(cls := "text", dataIcon := "赛", href := routes.Contest.show(c.contest.id))(c.contest.fullName),
            nbsp,
            c.boardName
          ),
          c.board.status.is(chess.Status.Created) option div(cls := "clock", dataTime := c.board.secondsToStart)(div(cls := "time")(c.board.startStatus)) /*,
          c.round.isStarted option div(cls := "clock running")("比赛中")*/
        )
      } orElse {
        game.contestId map { contestId =>
          st.section(cls := "game__contest-link")(
            contestLink(contestId)
          )
        }
      },
