...
object Study extends LilaController {
  ...
  def search(text: String, page: Int) = OpenBody { implicit ctx =>
    Reasonable(page) {
      if (text.trim.isEmpty)
        env.pager.all(
          ctx.me,
          Env.coach.studentApi.mineCoach _,
          Env.team.api.mineTeamOwner _,
          Order.default,
          page
        ) flatMap { pag =>
            negotiate(
  ...
  def chapterInfo(id: String, chapterId: String) = Open { implicit ctx =>
    env.api.byIdWithChapter(id, chapterId) flatMap {
      _.fold(notFound) {
        case WithChapter(study, chapter) => {
          if (study.isPrivate) {
            fuccess(Unauthorized(jsonError("您的研习是私有的！")))
          } else {
            Ok(env.jsonView.chapterInfo(study, chapter)).fuccess
          }
        }
      }
    }
