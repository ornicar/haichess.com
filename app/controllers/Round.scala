...
import lila.contest.{ Board, ContestRepo }
import lila.game.{ GameRepo, PgnDump, Pov, Game => GameModel }
      ...
      myTour(pov.game.tournamentId, true) flatMap { tour =>
        Game.preloadUsers(pov.game) zip
          ...
          Env.api.roundApi.player(pov, lila.api.Mobile.Api.currentVersion) map {
            case _ ~ simul ~ contest ~ chatOption ~ crosstable ~ playing ~ bookmarked ~ data =>
            ...
            (
              ...
              Env.api.roundApi.watcher(
                ...
                Env.bookmark.api.exists(pov.game, ctx.me)
                  ...
                  case tour ~ contest ~ simul ~ chat ~ crosstable ~ data ~ bookmarked => {
                    Ok(html.round.watcher(pov, data, tour, contest, simul, crosstable, userTv = userTv, chatOption = chat, bookmarked = bookmarked))
                  }
                }
    ...
    OptionFuResult(proxyPov(gameId, color)) { pov =>
      (pov.game.tournamentId ?? lila.tournament.TournamentRepo.byId) zip
        ...
        Env.bookmark.api.exists(pov.game, ctx.me) map {
          case tour ~ contest ~ simul ~ initialFen ~ crosstable ~ bookmarked =>
            Ok(html.game.bits.sides(pov, initialFen, tour, contest, crosstable, simul, bookmarked = bookmarked))
        }
    }
  }
