...
object Relation extends LilaController {
  ...
  def following(username: String, page: Int, game: Option[String]) = Open { implicit ctx =>
    Reasonable(page, 20) {
          ...
          RelatedPager(env.api.followingPaginatorAdapter(user.id), page) flatMap { pag =>
            negotiate(
              html = env.api countFollowers user.id map { nbFollowers =>
                Ok(html.relation.bits.following(user, pag, nbFollowers, game))
              },
  ...
  def followers(username: String, page: Int, game: Option[String]) = Open { implicit ctx =>
    Reasonable(page, 20) {
      OptionFuResult(UserRepo named username) { user =>
        Accessible(user) { _ =>
          RelatedPager(env.api.followersPaginatorAdapter(user.id), page) flatMap { pag =>
            negotiate(
              html = env.api countFollowing user.id map { nbFollowing =>
                Ok(html.relation.bits.followers(user, pag, nbFollowing, game))
              },
              api = _ => Ok(jsonRelatedPaginator(pag)).fuccess
            )
          }
  ...
  private def Accessible(u: UserModel)(f: UserModel => Fu[Result])(implicit ctx: Context) =
    grant(u, ctx.me) flatMap {
      case true => f(u)
      case _ => fuccess(Forbidden(html.relation.bits.forbidden(u)))
    }

  private def grant(u: UserModel, me: Option[UserModel]): Fu[Boolean] =
    if (me ?? Granter(_.UserSpy)) fuTrue
    else Env.pref.api.getPref(u) flatMap { pref =>
      pref.followShare match {
        case _ if me.contains(u) => fuTrue
        case lila.pref.Pref.FollowShare.EVERYBODY => fuTrue
        case lila.pref.Pref.FollowShare.FRIENDS => me ?? { m => lila.relation.Env.current.api.fetchAreFriends(u.id, m.id) }
        case lila.pref.Pref.FollowShare.NOBODY => fuFalse
      }
    }
