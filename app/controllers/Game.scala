...
object Game extends LilaController {
  ...
  def exportOne(id: String) = Open { implicit ctx =>
    OptionFuResult(GameRepo game id) { game =>
      if (game.playable) BadRequest("Only bots can access their games in progress. See https://haichess.com/api#tag/Chess-Bot").fuccess
      else {
        ...
        Env.api.gameApiV2.exportOne(game, config) flatMap { content =>
          Env.api.gameApiV2.filename(game, config.format) map { filename =>
            Ok(content).withHeaders(
              ...
              CONTENT_DISPOSITION -> s"attachment; filename=${new String(filename.getBytes(), "ISO8859-1")}"
            )
          }
        }
      }
          ...
          Api.GlobalLinearLimitPerUserOption(me) {
            ...
            WithVs(req) { vs =>
              ...
              Ok.chunked(Env.api.gameApiV2.exportByUser(config)).withHeaders(
                noProxyBufferHeader,
                CONTENT_TYPE -> gameContentType(config),
                CONTENT_DISPOSITION -> s"attachment; filename=haichess_${new String(username.getBytes(), "ISO8859-1")}_$date.${format.toString.toLowerCase}"
              ).fuccess
            }
          }
        }
