...
object User extends LilaController {
  ...
  private def Accessible(u: UserModel)(f: UserModel => Fu[Result])(implicit ctx: Context) =
    grant(u, ctx.me) flatMap {
      case true => f(u)
      case _ => fuccess(Forbidden(html.user.show.page.forbidden(u)))
    }
  ...
  private def grant(u: UserModel, me: Option[UserModel]): Fu[Boolean] =
    if (me ?? Granter(_.UserSpy)) fuTrue
      ...
      pref.homeShare match {
        case _ if me.contains(u) => fuTrue
        case lila.pref.Pref.HomeShare.EVERYBODY => fuTrue
        case lila.pref.Pref.HomeShare.FRIENDS => me ?? { m => lila.relation.Env.current.api.fetchAreFriends(u.id, m.id) }
        case lila.pref.Pref.HomeShare.NOBODY => fuFalse
      }
    } flatMap { bool =>
      if (bool) fuTrue
      else {
        Env.coach.studentApi.mineStudents(me) map (_.contains(u.id))
      }
    }
