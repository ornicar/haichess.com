...
import lila.app._
import lila.clazz.ClazzUser
import lila.common.{ ApiVersion, HTTPRequest, Lang, LilaCookie, Nonce }
...
private[controllers] trait LilaController
  ...
  protected def authenticationFailed(implicit ctx: Context): Fu[Result] =
    negotiate(
      html = if (HTTPRequest isSynchronousHttp ctx.req) fuccess {
        Redirect(routes.Auth.login) withCookies LilaCookie.session(Env.security.api.AccessUri, ctx.req.uri)
      }
      else fuccess(Results.Unauthorized("Login required")),
      ...
      Env.pref.api.getPref(me, ctx.req) zip {
        if (isPage) {
          ...
          Env.relation.online.friendsOf(me.id) zip
            Env.clazz.api.clazzUser(me.id) zip
            Env.team.api.nbRequests(me.id) zip
            Env.challenge.api.countInFor.get(me.id) zip
            Env.notifyModule.api.unreadCount(Notifies(me.id)).dmap(_.value) zip
            Env.mod.inquiryApi.forMod(me)
        } else fuccess {
          (((((OnlineFriends.empty, ClazzUser.empty), 0), 0), 0), none)
        }
      } map {
        case (pref, (onlineFriends ~ clazzUser ~ teamNbRequests ~ nbChallenges ~ nbNotifications ~ inquiry)) =>
          PageData(onlineFriends, clazzUser, teamNbRequests, nbChallenges, nbNotifications, pref,
  ...
  protected def XhrOrRedirectHome(res: => Fu[Result])(implicit ctx: Context) =
    ...
    else Redirect(routes.Home.home).fuccess
