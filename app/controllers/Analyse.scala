...
object Analyse extends LilaController {
  ...
  private def divider = lila.game.Env.current.divider

  def gameAnalysis(gameId: String) = Open { implicit ctx =>
    OptionFuResult(GameRepo game gameId) { game =>
      OptionFuResult(env.analyser.get(game)) { analysis =>
        val d = divider(game, none)
        fuccess(Ok(lila.analyse.JsonView.mobile(game, analysis)
          .add("division" -> Json.obj(
            "middle" -> d.middle,
            "end" -> d.end,
            "plies" -> d.plies
          ).some)))
    ...
    if (HTTPRequest isBot ctx.req) replayBot(pov)
      ...
      Game.preloadUsers(pov.game) >> RedirectAtFen(pov, initialFen) {
        (env.analyser get pov.game) zip
          ...
          Round.myContest(pov.game) zip
          Round.getWatcherChat(pov.game) zip
          (ctx.noBlind ?? Env.game.crosstableApi.withMatchup(pov.game)) zip
          Env.bookmark.api.exists(pov.game, ctx.me) zip
          Env.bookmark.api.tags(pov.game, ctx.me) zip
          Env.api.pgnDump(pov.game, initialFen, analysis = none, PgnDump.WithFlags(clocks = false)) flatMap {
            case analysis ~ analysisInProgress ~ simul ~ contest ~ chat ~ crosstable ~ bookmarked ~ tags ~ pgn =>
              Env.api.roundApi.review(pov, lila.api.Mobile.Api.currentVersion,
                ...
                withFlags = WithFlags(
                  ...
                  Ok(html.analyse.replay(
                    pov,
                    data.add("tags" -> tags.mkString(",").some),
                    initialFen,
