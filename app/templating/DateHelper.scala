...
trait DateHelper { self: I18nHelper =>
  ...
  def showPeriod(period: Period)(implicit ctx: Context): String =
    (periodFormatter(ctx) print period.normalizedStandard(periodType))
      ...
      .replaceAll("hours", "小时")
      .replaceAll("hour", "小时")
      .replaceAll("minutes", "分钟")
      .replaceAll("minute", "分钟")
  ...
  def showTime(seconds: Int)(implicit ctx: Context): String =
    s"${addZero(seconds / 60)}:${addZero(seconds % 60)}"

  def addZero(i: Int) = if (i < 10) s"0$i" else i;
