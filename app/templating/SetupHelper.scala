...
trait SetupHelper { self: I18nHelper =>
  ...
  def apptChoices(implicit ctx: Context) = List(("0", "否", none), ("1", "是", none))
  ...
  def translatedFromPosition(implicit ctx: Context): SelectChoice =
    (chess.variant.FromPosition.id.toString, chess.variant.FromPosition.name, chess.variant.FromPosition.title.some)
  ...
  def translatedSpeedChoices(implicit ctx: Context) = Speed.limited map { s =>
    ...
    (
      s.id.toString,
      s.name + " - " + trans.lessThanNbMinutes.pluralSameTxt(minutes),
      none
    )
  }
