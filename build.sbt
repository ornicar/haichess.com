...
lazy val modules = Seq(
  ...
  clazz, resource, member, contest, offlineContest, appt, calendar, recall, errors, home
)
...
lazy val puzzle = module("puzzle", Seq(
  common, memo, hub, history, db, user, rating, pref, tree, game, resource
)).settings(
...
lazy val coach = module("coach", Seq(
  ...
  libraryDependencies ++= provided(play.api, reactivemongo.driver, alipay)
)
...
lazy val home = module("home", Seq(
  common, db, memo, hub, socket, game, user, round, timeline, relation, playban, security
)).settings(
  libraryDependencies ++= provided(play.api, reactivemongo.driver)
)
...
lazy val importer = module("importer", Seq(common, game, puzzle, round)).settings(
  libraryDependencies ++= provided(play.api, reactivemongo.driver)
)
...
lazy val security = module("security", Seq(common, hub, db, user, i18n, slack, oauth)).settings(
  libraryDependencies ++= provided(play.api, scalatags, reactivemongo.driver, maxmind, hasher, aliyun)
)
...
lazy val study = module("study", Seq(
  common, db, hub, socket, memo, game, round, importer, notifyModule, relation, evalCache, explorer
)).settings(
  libraryDependencies ++= provided(play.api, reactivemongo.driver)
)
...
lazy val clazz = module("clazz", Seq(
  common, memo, db, user, notifyModule, team, puzzle, game
)).settings(
  libraryDependencies ++= provided(play.api, reactivemongo.driver, reactivemongo.iteratees)
)

lazy val resource = module("resource", Seq(
  common, memo, db, user
)).settings(
  libraryDependencies ++= provided(play.api, reactivemongo.driver, reactivemongo.iteratees)
)

lazy val member = module("member", Seq(
  common, hub, memo, db, user, coach
)).settings(
  libraryDependencies ++= provided(play.api, reactivemongo.driver, reactivemongo.iteratees, alipay)
)

lazy val contest = module("contest", Seq(
  common, hub, socket, memo, game, round, security, chat, quote, history, team, clazz, notifyModule, i18n
)).settings(
  libraryDependencies ++= provided(
    play.api, scalatags, reactivemongo.driver, reactivemongo.iteratees
  )
...
lazy val offlineContest = module("offlineContest", Seq(
  common, hub, socket, memo, game, round, security, chat, quote, history, team, clazz, notifyModule, i18n
)).settings(
  libraryDependencies ++= provided(
    play.api, scalatags, reactivemongo.driver, reactivemongo.iteratees
...
lazy val appt = module("appt", Seq(
  common, hub, socket, memo, game, round, contest, challenge, notifyModule, i18n
)).settings(
  libraryDependencies ++= provided(
    play.api, scalatags, reactivemongo.driver, reactivemongo.iteratees
  )
...
lazy val calendar = module("calendar", Seq(
  common, hub, socket, memo, game, round, notifyModule, i18n
)).settings(
  libraryDependencies ++= provided(
    play.api, scalatags, reactivemongo.driver, reactivemongo.iteratees
  )
...
lazy val recall = module("recall", Seq(
  common, hub, socket, memo, game, round, study, notifyModule, i18n
)).settings(
  libraryDependencies ++= provided(
    play.api, scalatags, reactivemongo.driver, reactivemongo.iteratees
  )
...
lazy val errors = module("errors", Seq(
  common, hub, socket, memo, game, round, puzzle, analyse, notifyModule, i18n
)).settings(
  libraryDependencies ++= provided(
    play.api, scalatags, reactivemongo.driver, reactivemongo.iteratees
  )
