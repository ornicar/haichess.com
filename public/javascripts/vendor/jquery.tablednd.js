...
!function ($, window, document, undefined) {
    ...
    let startEvent = 'touchstart mousedown',
    ...
    jQuery.tableDnD = {
        ...
        currentOrder: function () {
            let rows = this.currentTable.rows;
        ...
        autoScroll: function (mousePos) {
            let config = this.currentTable.tableDnDConfig,
        ...
        mousemove: function (e) {
            let dragObj = $($.tableDnD.dragObject),
            ...
            config.onDragClass && dragObj.addClass(config.onDragClass) || dragObj.css(config.onDragStyle);
        ...
        findDragDirection: function (x, y) {
            let sensitivity = this.currentTable.tableDnDConfig.sensitivity,
                oldX = this.oldX,
                oldY = this.oldY,
        ...
        processMouseup: function () {
            ...
            config.onDragClass && $(droppedRow).removeClass(config.onDragClass) || $(droppedRow).css(config.onDropStyle);

            this.dragObject = null;
            // Call the onDrop method if there is one
            config.onDrop && this.originalOrder !== this.currentOrder() /*&& $(droppedRow).hide().fadeIn('fast')*/ && config.onDrop(this.currentTable, droppedRow);
        ...
        serializeTable: function (table) {
            let result = "";
            let paramName = table.tableDnDConfig.serializeParamName || table.id;
            let rows = table.rows;
            for (let i = 0; i < rows.length; i++) {
