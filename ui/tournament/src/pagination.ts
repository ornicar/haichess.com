...
export function renderPager(ctrl: TournamentController, pag): MaybeVNodes {
  ...
  return pag.nbPages > -1 ? [
    ...
    ...(ctrl.searching ? [search.input(ctrl)] : [
      button('首页', 'W', () => ctrl.userSetPage(1), enabled && page > 1, ctrl),
      button('上一页', 'Y', ctrl.userPrevPage, enabled && page > 1, ctrl),
      h('span.page', (pag.nbResults ? (pag.from + 1) : 0) + '-' + pag.to + ' / ' + pag.nbResults),
      button('下一页', 'X', ctrl.userNextPage, enabled && page < pag.nbPages, ctrl),
      button('末页', 'V', ctrl.userLastPage, enabled && page < pag.nbPages, ctrl),
