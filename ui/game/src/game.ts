...
export function playable(data: GameData): boolean {
  return data.game.status.id > status.ids.created && data.game.status.id < status.ids.aborted && !imported(data) && (!data.game.appt || (data.game.appt && data.game.apptComplete));
}
...
export function mandatory(data: GameData): boolean {
  return !!data.tournament || !!data.contest || !!data.simul;
}
