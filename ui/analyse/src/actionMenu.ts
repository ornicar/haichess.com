  ...
  return h('div.action-menu',
    ...
    .concat([
      ...
      canContinue ? h('div.continue-with.none.g_' + d.game.id, [
        h('a.button', {
          attrs: {
            href: d.userAnalysis ? '/lobby?fen=' + ctrl.encodeNodeFen() + '#ai' : contRoute(d, 'ai') + '?fen=' + ctrl.node.fen,
            rel: 'nofollow'
          }
        }, noarg('playWithTheMachine')),
        h('a.button', {
          attrs: {
            href: d.userAnalysis ? '/lobby?fen=' + ctrl.encodeNodeFen() + '#friend' : contRoute(d, 'friend') + '?fen=' + ctrl.node.fen,
            rel: 'nofollow'
          }
