...
export default function(element: HTMLElement, ctrl: AnalyseCtrl) {
  ...
  if (!li.AnalyseNVUI) {
    ...
    li.pubsub.on('analysis.change', (fen: Fen, _, mainlinePly: Ply | false) => {
      let $acplChart = $("#acpl-chart");
      if (fen && fen !== lastFen) {
        inputFen.value = fen;
        lastFen = fen;
      }
      if ($acplChart.length) {
        if (mainlinePly != $acplChart.data('lastPly')) {
          if (mainlinePly === false) li.pubsub.emit('analysis.chart.unselect', 'acplChart');
          else {
            const turn = mainlinePly - 1 - data.game.startedAtTurn;
            li.pubsub.emit('analysis.chart.select', 'acplChart', 0, turn);
          }
        }
        $acplChart.data('lastPly', mainlinePly);
      }
      if ($timeChart.length) {
        if (mainlinePly != $timeChart.data('lastPly')) {
          if (mainlinePly === false) li.pubsub.emit('analysis.chart.unselect', 'timeChart');
          else {
            const white = mainlinePly % 2 !== 0;
            const serie = white ? 0 : 1;
            const turn = Math.floor((mainlinePly - 1 - data.game.startedAtTurn) / 2);
            li.pubsub.emit('analysis.chart.select', 'timeChart', serie, turn);
          }
        }
        $timeChart.data('lastPly', mainlinePly);
      }
    });
    li.pubsub.on('analysis.server.progress', (d: AnalyseData) => {
      if (!li.advantageChart) {
        ...
        li.pubsub.emit('analysis.chart.change', 'acplChart', d);
      }
