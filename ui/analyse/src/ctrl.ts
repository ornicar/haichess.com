...
export default class AnalyseCtrl {
  ...
  constructor(readonly opts: AnalyseOpts, readonly redraw: Redraw) {
    ...
    this.data = opts.data;
    this.isCoach = opts.isCoach;
    this.isTeam = opts.isTeam;
    this.canCreatePrivate = opts.canCreatePrivate;
  ...
  deleteNode(path: Tree.Path): void {
    ...
    if ((count.nodes >= 10 || count.comments > 0) && !confirm(
      '删除 ' + util.plural('步', count.nodes) + (count.comments ? ' 和 ' + util.plural('评注', count.comments) : '') + '?'
    )) return;
  ...
  setTag(v: string) {
    ...
    $.ajax({
      method: 'POST',
      url: '/bookmark/' + this.data.game.id + '/setTag',
      data: {
        tags: v
      }
    }).then(function() {
