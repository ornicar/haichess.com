...
export function ctrl(root: AnalyseCtrl, chapterId: () => string): ServerEvalCtrl {
  ...
  li.pubsub.on('analysis.change', (_fen: string, _path: string, mainlinePly: number | false) => {
    ...
    if (el) {
      const $chart = $(el);
      if ($chart.length) {
        if (lp === false) li.pubsub.emit('analysis.chart.unselect', 'acplChart');
        else {
          const turn = lp - 1 - root.tree.root.ply;
  ...
  return {
    ...
    chapterId,
    onMergeAnalysisData() {
      if (li.advantageChart) li.pubsub.emit('analysis.chart.change', 'acplChart', root.data);
    },
    request() {
