...
const autoScroll = throttle(150, (ctrl, el) => {
  ...
  if (!target) {
    cont.scrollTop = ctrl.vm.currentPath === treePath.root ? 0 : 99999;
    return;
  }
...
function pathContains(ctx, path) {
  return treePath.contains(ctx.ctrl.vm.currentPath, path);
}
...
function gameGlyph(ctx, node): VNode | undefined {
  switch (node.recall) {
    case 'win':
      return renderGlyph({
        name: '最佳着',
...
function renderVariationMoveOf(ctx, node, opts): VNode {
  ...
  const path = opts.parentPath + node.id;
  const active = path === ctx.ctrl.vm.currentPath;
  const classes: any = {
    active,
    ...
    hook: {
      ...
      postpatch: (_, vnode) => {
        if (ctrl.vm.autoScrollNow) {
          ...
          if (ctrl.vm.currentPath !== treePath.root) autoScroll(ctrl, vnode.elm as HTMLElement);
