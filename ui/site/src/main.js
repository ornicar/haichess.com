(function() {
  ...
  $.extend(true, lichess.StrongSocket.defaults, {
    events: {
      following_onlines: function(d, all) {
        $('#friend_box').friends("set", all.d, all.playing, all.studying, all.patrons, all.heads);
      },
      following_enters: function(d, all) {
        $('#friend_box').friends('enters', all.d, all.playing, all.studying, all.patron, all.head);
      },
      ...
      deployPre: function() {
        ...
        $('body').append(
            ...
            '<div data-icon="j" class="is3 text">服务器将在1分钟内进行维护</div>' +
      ...
      deployPost: function() {
        ...
        $('body').append(
          '<div id="announce" class="announce">' +
          '<div data-icon="j" class="is3 text">服务器正在维护.</div>' +
          '<div class="actions"><a class="close">X</a></div>' +
          '</div>').find('#announce .close').click(function() { $('#announce').remove(); });
        lichess.socket.disconnect(function() {
          $('#announce').remove();
          $('body').append(
            '<div id="announce" class="announce">' +
            '<div data-icon="E" class="is3 is-green text">服务器维护完成</div>' +
            '<div class="actions"><a class="close">X</a></div>' +
            '</div>').find('#announce .close').click(function() { $('#announce').remove(); });
        });
      },
      tournamentReminder: function(data) {
        ...
        $('body').append(
          ...
          '<div class="actions">' +
          '<a class="withdraw text" href="' + url + '/withdraw" data-icon="Z">暂停</a>' +
          '<a class="text" href="' + url + '" data-icon="G">加入</a>' +
          '</div></div>'
      ...
      contestReminder: function(data) {
        var u = location.href;
        if (u.substring(u.length - 8) == data.gameId) return;

        $('#announce').remove();
        var url = '/' + data.gameId + '/' + data.color;
        $('body').append(
            '<div id="announce">' +
            '<a data-icon="赛" class="text" href="' + url + '">' + data.name + '</a>' +
            '<div class="actions">' +
           /* '<a class="withdraw text" href="' + url + '/withdraw" data-icon="Z">暂停</a>' +*/
            '<a class="text" href="' + url + '" data-icon="G">进入</a>' +
            '</div></div>'
        );
      },
      gameApptReminder: function(data) {
        var u = location.href;
        if (u.substring(u.length - 8) == data.gameId) return;
        ...
        $('body').append(
            '<div id="announce">' +
            '<a data-icon="U" class="text" href="' + url + '">朋友约棋（'+ data.opponent +'）</a>' +
            '<div class="actions">' +
            '<a class="text" href="' + url + '" data-icon="G">进入</a>' +
            '</div></div>'
        );
      ...
      coachCertifyPassed: function(d) {
        if(location.href.indexOf('coach/certify') != -1) {
          location.reload();
        }
      }
    },
  ...
  lichess.readServerFen = t => t ? atob(lichess.reverse(t)) : '';
    ...
    return lichess.loadScript('javascripts/vendor/typeahead.jquery.min.js').done(function() {
      $input.typeahead({
        ...
        source: function(query, _, runAsync) {
          if (query.trim().match(/^[a-z\u4e00-\u9fa5][\u4e00-\u9fa5\w-]{0,18}[a-z0-9\u4e00-\u9fa5]$/i)) $.ajax({
        ...
        displayKey: 'name',
        templates: {
          empty: '<div class="empty">匹配不到用户</div>',
          pending: lichess.spinnerHtml,
          suggestion: function(o) {
            var tag = opts.tag || 'a';
            var headSrc = o.head ? '/image/' + o.head : $('body').data('asset-url') + '/assets/images/head-default-64.png';
            var patron = o.patron ? '<i class="patron"></i>' : '';
            var title = o.title ? '<span class="title">' + o.title + '</span>' : '';
            var uHtml =
            '<' + tag + ' class="user-link ulpt' + (o.online ? ' online' : ' offline') + '" ' + (tag === 'a' ? '' : 'data-') + 'href="/@/' + o.name + '">' +
            '<div class="head-line">' +
  ...
  $(function() {
    if (lichess.home) startHome(lichess.home);
    else if (lichess.analyse) LichessAnalyse.boot(lichess.analyse);
    ...
    else if (lichess.puzzleRush) startPuzzleRush(lichess.puzzleRush);
    else if (lichess.recall) startRecall(lichess.recall);
    ...
    lichess.requestIdleCallback(function() {
      ...
      lichess.loadInfiniteScroll = function(el) {
        $(el).each(function() {
          ...
          $scroller.parent().append($('<button class="inf-more button button-empty">更多</button>').on('click', function() {
      ...
      $('a.delete, input.delete').click(() => confirm('删除？'));
      $('input.confirm, button.confirm').click(function() {
        return confirm($(this).attr('title') || '确认操作？');
      });
  ...
  lichess.widget("friends", (function() {
    ...
    var renderUser = function(user) {
      var headSrc = user.head ? '/image/' + user.head : $('body').data('asset-url') + '/assets/images/head-default-64.png';
      var head = '<img class="head" src ="' + headSrc + '">';
      var icon = '<i class="line"></i>';
      var headLine = '<div class="head-line">'+ head + icon +'</div>';
      var titleTag = user.title ? ('<span class="title"' + (user.title === 'BOT' ? ' data-bot' : '') + '>' + user.title + '</span>&nbsp;') : '';
      var patron = user.patron ? '<i class="patron"></i>' : '';
      var badges = '<span class="badges">'+ patron +'</span>';
      var url = '/@/' + user.name;
      ...
      return '<div><a class="user-link online ulpt" data-pt-pos="nw" href="' + url + '">' + headLine + titleTag + user.name + badges + '</a>' + rightButton + '</div>';
    };
    return {
      _create: function() {
        var self = this;
        ...
        function headList() {
          return el.data('heads') ? el.data('heads') : [];
        }
        self.set(
          dataList('preload'),
          dataList('playing'),
      ...
      set: function(online, playing, studying, patrons, heads) {
        this.users = {};
        var i;
        ...
        for (i in heads) this.insert(heads[i].u).head = heads[i].h;
        this.repaint();
      },
      enters: function(titleName, playing, studying, patron, head) {
    ...
    _create: function() {
      ...
      var tick = function() {
        ...
        if (remaining <= 0) {
          ...
          if(typeof self.options.stopped == 'function') {
            self.options.stopped(self);
          }
        }
  ...
  lichess.memberIntro = function() {
    lichess.loadCssPath('member.intro');
    $.get({ url: '/member/intro' }).then(function(html) {
      $.modal($(html));
  ...
  function startHome(cfg) {
    let element = document.querySelector('div.home-header-content');
    let home;
    lichess.socket = lichess.StrongSocket('/home/socket/v4', false, {
      options: {
        name: 'home'
      },
      receive: function(t, d) {
        home.socketReceive(t, d);
      }
    });
    cfg.socketSend = lichess.socket.send;
    cfg.element = element;
    home = LichessHome.start(cfg);
  }
  ...
  function startPuzzleRush(cfg) {
    var puzzleRush;
    cfg.element = document.querySelector('main.puzzleRush');
    lichess.socket = lichess.StrongSocket('/socket/v4', false, {
      receive: function(t, d) {
        puzzleRush.socketReceive(t, d);
      }
    });
    cfg.socketSend = lichess.socket.send;
    puzzleRush = LichessPuzzleRush.default(cfg);
  }
  ...
  function startRecall(cfg) {
    var recall;
    cfg.element = document.querySelector('main.recall');
    lichess.socket = lichess.StrongSocket('/socket/v4', false, {
      receive: function(t, d) {
        recall.socketReceive(t, d);
      }
    });
    cfg.socketSend = lichess.socket.send;
    recall = LichessRecall.default(cfg);
  }
