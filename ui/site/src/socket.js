...
/*
  const baseUrls = (
    d => [d].concat((d.includes('haichess.com') ? [5, 6, 7, 8, 9] : []).map(port => d + ':' + (9020 + port)))
  )(options.remoteSocketDomain || document.body.getAttribute('data-socket-domain'));
*/
  // 目前只有一个server
  let baseUrls = [options.remoteSocketDomain || document.body.getAttribute('data-socket-domain')];
