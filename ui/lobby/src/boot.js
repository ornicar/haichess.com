module.exports = function(cfg, element) {
  var pools = [{id:"1+1",lim:1,inc:1,perf:"闪电棋"},{id:"3+2",lim:3,inc:2,perf:"超快棋"},{id:"5+0",lim:5,inc:0,perf:"超快棋"},{"id":"5+3","lim":5,"inc":3,"perf":"超快棋"},{id:"10+0",lim:10,inc:0,perf:"快棋"},{"id":"15+10","lim":15,"inc":10,"perf":"中速棋"},{id:"25+10",lim:25,inc:10,perf:"中速棋"},{id:"50+10",lim:50,inc:10,perf:"慢棋"}];
  var lobby;
  ...
  function prepareForm($modal) {
    ...
    var $apptSelect = $form.find('#sf_appt');
      ...
      switch ($variantSelect.val()) {
        ...
        case '3':
          if (timeMode == '1') {
            ...
            if (time < 120) key = 'ultraBullet';
            else if (time < 600) key = 'bullet';
            else if (time < 1200) key = 'blitz';
            else if (time < 3000) key = 'rapid';
            else key = 'classical';
    ...
    $apptSelect.on('change', function() {
      let v = $(this).val();
      if(v == 1) {
        $('.appt_pick').removeClass('none');
      } else {
        $('.appt_pick').addClass('none');
      }
    }).trigger('change');
    $form.find('.flatpickr').flatpickr({minuteIncrement: 1});
  ...
  if (['#ai', '#friend', '#hook'].includes(location.hash)) {
    $startButtons
      ...
      .each(function() {
        ...
        if(href.indexOf('?') > 0) {
          href = href + location.search.replace('?', '&');
        } else {
