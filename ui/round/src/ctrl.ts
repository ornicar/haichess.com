...
export default class RoundController {
  ...
  constructor(readonly opts: RoundOpts, readonly redraw: Redraw) {
    ...
    setTimeout(this.showContestExpiration, 350);
  ...
  private showExpiration = () => {
    if (!this.data.expiration || this.data.contest) return;
    this.redraw();
    setTimeout(this.showExpiration, 250);
  };

  private showContestExpiration = () => {
    if (!this.data.contestExpiration) return;
    this.redraw();
    setTimeout(this.showContestExpiration, 250);
  };
  ...
  apiMove = (o: ApiMove): void => {
    ...
    if (!!this.data.contestExpiration) {
      if (this.data.steps.length > 2) this.data.contestExpiration = undefined;
      else this.data.contestExpiration.movedAt = Date.now();
    }
    this.redraw();
