...
export function backToTournament(ctrl: RoundController): VNode | undefined {
  ...
  return (d.tournament && d.tournament.running) ? h('div.follow-up', [
    ...
    h('form', {
      ...
      h('button.text.fbt.weak', util.justIcon('Z'), '暂停')
    ]),
    analysisButton(ctrl)
  ]) : undefined;
}
...
export function followUp(ctrl: RoundController): VNode {
  const d = ctrl.data,
    rematchable = !d.game.rematch && (status.finished(d) || status.aborted(d)) && !d.tournament && !d.contest && !d.simul && !d.game.boosted,
