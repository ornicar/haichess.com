...
export default function(opts: RoundOpts): void {
  ...
  if (data.contest) $('body').data('contest-id', data.contest.id);
  ...
  function startApptClock() {
    $('.game__appt .clock').each(function(this: HTMLElement) {
      $(this).clock({
  ...
  function startContestClock() {
    $('.game__contest .clock').not('.running').each(function(this: HTMLElement) {
      $(this).clock({
        time: parseFloat($(this).data('time'))
      });
    });
