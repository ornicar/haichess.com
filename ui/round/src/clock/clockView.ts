...
export function renderClock(ctrl: RoundController, player: Player, position: Position) {
  ...
  return h('div.rclock.rclock-' + position, {
    ...
    contestRound(ctrl, player.color, position)
  ]);
}
...
function contestRound(ctrl: RoundController, color: Color, position: Position) {
  const d = ctrl.data;
  return (d.contest && d.contest.players) ?
      h('div.contest-round.' + position, {
        attrs: {title: '序号'}
      }, '#' + d.contest.players[color]) : null;
}